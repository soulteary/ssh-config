...
func TestCheckConvertArgvValid(t *testing.T) {
	tests := []struct {
		...
		{
			name:       "Document path with ToSSH",
			args:       Cmd.Args{ToSSH: true, DocumentPath: "/home/alice/.ssh/config"},
			wantResult: true,
			wantDesc:   "",
		},
		{
			name:       "Document path with structured output",
			args:       Cmd.Args{ToYAML: true, DocumentPath: "/home/alice/.ssh/config"},
			wantResult: false,
			wantDesc:   "-document-path is only valid with -to-ssh",
		},
		{
			name:       "Document path in legacy mode",
			args:       Cmd.Args{ToSSH: true, Legacy: true, DocumentPath: "/home/alice/.ssh/config"},
			wantResult: false,
			wantDesc:   "-document-path is not supported with -legacy",
		},
		{
...
func TestParseArgs(t *testing.T) {
	tests := []struct {
		...
		{
			name: "Set all flags",
			args: []string{"-to-yaml", "-to-ssh", "-to-json", "-lossless", "-legacy", "-src", "source.txt", "-dest", "destination.txt", "-document-path", "/home/alice/.ssh/config", "-help", "-version"},
			expected: Cmd.Args{
				ToYAML:       true,
				ToSSH:        true,
				ToJSON:       true,
				...
				DocumentPath: "/home/alice/.ssh/config",
				ShowHelp:     true,
				Version:      true,
			},
		},
