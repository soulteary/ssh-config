...
- `-src`: 指定输入文件；显式路径优先于管道标准输入，省略时无损模式读取 `~/.ssh/config`，旧模式扫描 `~/.ssh`（其中只读取 `config` 与 `config.d/*`，避免把放在旁边的无关文件一并读入；用 `-src` 显式指定的目录仍会整目录扫描，不限文件名）。管道标准输入若不含任何非空白字节会被拒绝，避免上游命令失败时把 `-dest` 覆盖成空文件
